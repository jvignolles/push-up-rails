...
private
  ...
  def clear_table(*table_names)
    table_names.flatten.reject(&:blank?).each do |table_name|
      cnx.execute "TRUNCATE #{cnx.quote_table_name table_name}"
      trans_table = "#{table_name.singularize}_translations"
      if cnx.tables.include?(trans_table)
        cnx.execute "TRUNCATE #{cnx.quote_table_name trans_table}"
      end
    end
  end
...
# raise # Comment out to avoid big mistakes!
