...
module AdminActions
  ...
  def tabs
    ...
    h[:images] = { text: "Images", icon: "picture" } if model.const_defined?(:IMAGE_KINDS) && model::IMAGE_KINDS.present?
    h[:seo] = { text: "Référencement", icon: "cog" } if @item.respond_to?(:seo_title)
...
protected
  ...
  def search(search = {})
    ...
    # TODO: add includes (ex: .includes(:images))
    scoped
  end
